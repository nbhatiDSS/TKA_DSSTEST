...
page 70056 "Posted Booking "
{
        ...
        area(processing)
        {
            ...
            action("Update PAS Ref No.")
            {
                ...
                var
                    ...
                    SalesSetup: record "Sales & Receivables Setup";
                    GlobalIris: record GlobalIRIS;
                    SalesInvHeader: Record "Sales Invoice Header";
                begin
                    SalesSetup.get;
                    salessetup.ToText := rec."PAS Ref No.";
                    SalesSetup.Modify();
                    Commit();
                    Param.setreportid(11);
                    Param.runmodal;
                    SalesSetup.get;
                    if SalesSetup.ToText = '' then begin
                        if Confirm('Are you sure you want to remove PAS Ref?') then
                            rec."PAS Ref No." := SalesSetup.ToText;
                        rec.Modify();
                        Message('PAS Ref Updated');
                    end else begin
                        GlobalIris.reset;
                        GlobalIris.SetFilter("SalesOrder No.", '<>%1', rec."Original So No");
                        GlobalIris.setfilter("PAS Ref No.", '%1', SalesSetup.ToText);
                        if GlobalIris.findset then error('PAS Ref No. alreadys used');
                        SalesInvHeader.reset;
                        SalesInvHeader.SetFilter("No.", '<>%1', rec."No.");
                        SalesInvHeader.setfilter("PAS Ref No.", '%1', SalesSetup.ToText);
                        if SalesInvHeader.findset then error('PAS Ref No. alreadys used');
                        rec."PAS Ref No." := SalesSetup.ToText;
                        rec.Modify();
                        Message('PAS Ref Updated');
                    end;
    ...
    begin
        ...
        rec.CreditStatus := '';
        cle.Reset();
        if cle.get(rec."Cust. Ledger Entry No.") then begin
            ...
            repeat
                if (cle."Document Type" = cle."Document Type"::payment) OR (cle."Document Type" = cle."Document Type"::" ") then begin
                    ...
                    paidAmt += abs(cle.Amount);
                end
                else if (cle."Document Type" = cle."Document Type"::"Credit Memo") then begin
                    cle.CalcFields(Amount);
                    creditedAmt += abs(cle.Amount);
                    rec.CreditStatus := cle."Reason Code";
                end;
            until cle.next = 0;
            rec.CalcFields("Amount Including VAT");
            cle.Reset();
            cle.get(rec."Cust. Ledger Entry No.");
            if not cle.open then begin
                if paidAmt > 0 then
                    rec."Paid Amount" := rec."Amount Including VAT" - creditedAmt
            end
            else
                rec."Paid Amount" := paidAmt;
            rec.Modify()
        end;
    end;


    // local procedure UpdateDocAndAmount1()
    // var
    //     paidAmt: Decimal;
    //     creditedAmt: Decimal;
    //     Custled: Record "Cust. Ledger Entry";
    //     detCustLed, detCustLed1 : Record "Detailed Cust. Ledg. Entry";
    // begin
    //     paidAmt := 0;
    //     creditedAmt := 0;
    //     rec."Paid Amount" := 0;
    //     Custled.Reset();
    //     Custled.SetCurrentKey("Document Type", "Document No.");
    //     Custled.SetFilter("Document Type", '%1', Custled."Document Type"::Invoice);
    //     Custled.SetFilter("Document No.", '%1', rec."No.");
    //     if Custled.FindFirst() then begin
    //         GetAppliedentries(Custled);
    //         if custled.FindFirst() then
    //             repeat
    //                 detCustLed.SetFilter("Cust. Ledger Entry No.", '%1', Custled."Entry No.");
    //                 detCustLed.SetFilter("Entry Type", '%1', detCustLed."Entry Type"::Application);
    //                 detCustLed.SetFilter("Document Type", '%1', detCustLed."Document Type"::Invoice);
    //                 detCustLed.SetFilter("Document No.", '%1', rec."No.");
    //                 detCustLed.SetFilter(Unapplied, '%1', false);
    //                 if detCustLed.FindFirst() then begin
    //                     case Custled."Document Type" of
    //                         "Gen. Journal Document Type"::" ", "Gen. Journal Document Type"::Payment:
    //                             begin
    //                                 paidAmt += abs(detCustLed.Amount);
    //                             end;
    //                         "Gen. Journal Document Type"::"Credit Memo":
    //                             begin
    //                                 creditedAmt += abs(detCustLed.Amount);
    //                             end;
    //                     end;
    //                 end;
