...
page 70240 "Posted Sales Invoices Cust1"
{
    ...
    Caption = 'Posted Sales Invoices - TKA';
    CardPageID = 70056;
    Editable = false;
    PageType = List;
    ...
    layout
    {
        area(content)
        {
            repeater(Control1)
            {
                ...
                {
                    ApplicationArea = Suite;
                    ToolTip = 'Specifies the number of the related quote.';
                    Visible = true;
                }
                ...
                {
                    Caption = 'Flexi Pass 6';
                    ApplicationArea = All;
                }
                field("Flexi Pass 12"; Rec."Flexi Pass 12")
                {
                    ApplicationArea = All;
                }
                field("Flexi Pass 2"; Rec."Flexi Pass 2")
                {
                    ApplicationArea = All;
                }
    ...
    actions
    {
        ...
        area(processing)
        {
            ...
            group(Functions)
            {
                ...
                action(AllocateVoucher)
                {
                    ApplicationArea = All;
                    Caption = 'Allocate Voucher', comment = 'NLB="YourLanguageCaption"';
                    Image = Allocate;

                    trigger OnAction()
                    VAR
                        AllocateVoucher: Record 427;
                        SrNo: Integer;
                        Contact: Record 5050;
                        SalesInvoiceLine: Record 113;
                    BEGIN
                        IF UPPERCASE(USERID) IN ['MARTHA.FOLKES', 'BIJU.KURUP', 'TRIZA.BEHAL', 'GAGANDEEP.SAHOTA', 'MANISHA.PAL'] THEN BEGIN
                            IF NOT CONFIRM('Are you sure to Allocate Voucher for these Invoices') THEN
                                EXIT;

                            AllocateVoucher.SETFILTER(AllocateVoucher."IC Transaction No.", '<>%1', 0);
                            IF AllocateVoucher.FIND('+') THEN
                                SrNo := AllocateVoucher."IC Transaction No." + 1
                            ELSE
                                SrNo := 1;

                            AllocateVoucher.INIT;
                            CurrPage.SETSELECTIONFILTER(Rec);
                            Rec.FIND('-');
                            REPEAT
                                IF rec."Voucher Information" = '' THEN BEGIN
                                    AllocateVoucher."Event Number" := '';
                                    AllocateVoucher."Course Header" := '';

                                    SalesInvoiceLine.SETFILTER(SalesInvoiceLine."Document No.", rec."No.");
                                    SalesInvoiceLine.SETFILTER(SalesInvoiceLine."Course Header", 'PMP-E-LEARN');
                                    IF SalesInvoiceLine.FIND('-') THEN
                                        REPEAT
                                            SalesInvoiceLine.CALCFIELDS("Contact Name");

                                            AllocateVoucher."Event Number" := SalesInvoiceLine."Event Header";
                                            AllocateVoucher."Course Header" := SalesInvoiceLine."Course Header";


                                            AllocateVoucher."IC Transaction No." := SrNo;
                                            AllocateVoucher."Document No." := Rec."No.";
                                            AllocateVoucher."Posting Date" := Rec."Posting Date";
                                            AllocateVoucher."Customer No" := rec."Sell-to Customer No.";
                                            AllocateVoucher."Customer Name" := rec."Sell-to Customer Name";

                                            AllocateVoucher."Contact No" := SalesInvoiceLine."Contact No.";
                                            AllocateVoucher."Contact Name" := SalesInvoiceLine."Contact Name";

                                            AllocateVoucher."Contact Email" := '';
                                            IF Contact.GET(SalesInvoiceLine."Contact No.") THEN BEGIN
                                                AllocateVoucher."Contact Email" := Contact."E-Mail";
                                            END;

                                            AllocateVoucher."Paid Status" := rec.PaymentStatus3;
                                            Rec.CALCFIELDS(Amount);
                                            AllocateVoucher."Amount Paid" := rec.Amount;


                                            AllocateVoucher.Description := USERID + ' ' + FORMAT(TODAY);
                                            AllocateVoucher."Line Amount" := SalesInvoiceLine.Amount;

                                            AllocateVoucher.INSERT;
                                            rec."Voucher Information" := 'Voucher Allocated';
                                            Rec.MODIFY;
                                            SrNo := SrNo + 1;
                        ...
                        END
                        ELSE
                            ERROR('You do not have permission to run Allocate Voucher Report');
                    END;
        ...
        area(Promoted)
        {
            ...
            {
                Caption = 'Functions';
                actionref(AllocateVoucher_Promoted; AllocateVoucher)
                {
                }
            }
    ...
    begin
        ...
        if not HasFilters then begin
            rec.SetCurrentKey("Posting Date");
            rec.SetFilter("Posting Date", '>%1', CalcDate('-2Y', Today));
        end;
