page 70081 "Bank Transactions Paid-In "
{
    ...
    actions
    {
        area(processing)
        {
            ...
            group(btnFunctionsMain)
            {
                ...
                {
                    ApplicationArea = All;
                    Caption = 'Process G/Ls';

                    trigger OnAction()
                    begin
                        CreateAndPostGL(format(rec."Transaction Type"));
                    end;
                }
            }
        }
    ...
    var
        lToDO: Record "5080";
    begin
        if rec.Amount <> rec."Total Sales Amount" then
            AmountStyleExpr := 'Unfavorable'
        else
    ...
    procedure CreateAndPostGL(var BTIB: Record "Bank Transaction Import Buffer")
    var
        ...
        EventCode: code[20];
    begin
        EventCode := 'EVE0000000';
        GenJrnl.SetFilter("Journal Template Name", '%1', 'PAYMENT');
        GenJrnl.SetFilter("Journal Batch Name", '%1', 'BANK');
        if GenJrnl.FindSet() then GenJrnl.DeleteAll();
        if BTIB."Payment Journal Created" then Error('Payment Joural is already created');
        if BTIB.FindFirst() then
            repeat
                if BTIB."Bank Acc. No." = '' then
                    error('Bank Account is missing.')
                else begin
                    Bank.Reset();
                    Bank.SetFilter("Bank Account No.", BTIB."Bank Acc. No.");
                    if Bank.FindFirst() then begin
                        if GLAcc.get(BTIB."G/L Account") then begin
                            GenJrnl.Init();
                            GenJrnl.Validate("Journal Template Name", 'PAYMENT');
                            GenJrnl.Validate("Journal Batch Name", 'BANK');
                            if GenJournalBatch.get('PAYMENT', 'BANK') then GenJrnl.Validate("Document No.", NoseriesMgmt.GetNextNo(GenJournalBatch."No. Series"));
                            GenJrnl.Validate("Posting Date", Today);
                            GenJrnl.Validate("Line No.", 10000);
                            ...
                            GenJrnl.Validate("Account No.", GLAcc."No.");
                            GenJrnl.Validate(Amount, BTIB.Amount);
                            GenJrnl.Validate("Bal. Account Type", GenJrnl."Bal. Account Type"::"Bank Account");
                            GenJrnl.Validate("Bal. Account No.", bank."No.");
                            if CopyStr(GenJrnl."Account No.", 1, 1) = '8' then begin
                                GenJrnl.Validate("Shortcut Dimension 1 Code", 'HEADOFFICE');
                                GenJrnl.Validate("Shortcut Dimension 2 Code", 'HEAD OFFICE');
                                GenJrnl.ValidateShortcutDimCode(3, EventCode);
                            end;
                            GenJrnl.insert();
                            BTIB."Payment Journal Created" := true;
                            BTIB.Modify();
    ...
    local procedure CreateAndPostGL(transactionType: text[15])
    var
        GenJrnl: record "Gen. Journal Line";
        GLAcc: record "G/L Account";
        Bank: record "Bank Account";
    ...
    begin
        ...
        BTIB.SetCurrentKey("G/L Account");
        BTIB.SetFilter("Transaction Type", transactionType);
        BTIB.SetFilter("G/L Account", '<>%1', '');
        BTIB.SetFilter("Payment Journal Created", '%1', false);
        if BTIB.FindFirst() then
            repeat
                if BTIB."Bank Acc. No." = '' then
                    error('Bank Account is missing.')
